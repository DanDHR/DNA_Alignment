...
def records_distribution(target_record, breeds_record, scores) ->list:
    ...
    """
    distr = DistributionCalc(target_record, breeds_record, scores)
    return distr.p_values_get()
...
if __name__ == '__main__':
    ...
    scores = find_best_pairwise_alignment(target_record[0],breeds_records)
    print(records_distribution(target_record[0], breeds_records, scores))

    alignment = AlignIO.read("external_data/dog_breeds_alignment.fa", "fasta")
    phylo_tree = PhylogeneticTreeBuilder(alignment)
